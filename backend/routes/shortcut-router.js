...
router.route('/getorpost').get((req, res) => {
  ...
  try {
    ...
    con.query(hasEmail, [id], async (err, result) => {
      if (result.length === 0) {
        return res.json(
          responseMessage('error', 'Client not created'),
        );
      }
      console.log(result);
...
router.route('/getorpost').post(async (req, res) => {
  const { textarea, id, rating, dateData, username, link } =
    req.body;
  function emailGenerator(start, name, textarea) {
    return {
      subject:
        'subject  New Review Submitted via Your QR Code System',
...
Phone: 832 266 7209
  ...
  try {
    const hasClientId =
      'SELECT * FROM shortcutsql WHERE unique_id = ?';
  ...
  try {
    ...
    con.query(hasClientId, [id], async (err, result) => {
      ...
      function emailGenerator() {
        return {
          subject:
            'Successful Review Submission via Your QR Code System',
...
router.route('/post').get((req, res) => {
  ...
  try {
    ...
    con.query(hasClientId, [id], async (err, sqlData) => {
      ...
      return res.json(
        responseMessage('success', 'ok', { data: sqlData }),
      );
    });
  } catch (err) {
    return res.json(
...
router.route('/:id').delete((req, res) => {
  ...
  try {
    const isVarify = verifyToken(token);
    const hasEmail =
      'UPDATE shortcutsql SET valid = ? WHERE unique_id = ?';
    con.query(
      hasEmail,
      [false, id],
      async (err, result) => {
...
router
  .route('/')
  .put(upload.single('imageURL'), function (req, res) {
    try {
      const { custome_url, name, email, imageURL } =
        req.body;
      const filePath =
        req.file === undefined
          ? imageURL
