...
export const deleteMessage = (event: any) =>
  api.delete(url.DELETE_MESSAGE, { headers: { event } });

// email
export const getMailsListsId = (id: string) =>
...
export const getselectedmails = (selectEmailId: any) =>
  api.create(url.GET_SELECTE_MAIL, { params: selectEmailId });
export const getUpdateMail = (mail: any) => api.put(url.UPDATE_MAIL, mail);
export const setFolderOnSelectedMails = (data: any) =>
  api.delete(url.SET_FOLDER_SELECTED_MAILS, { headers: { data } });
export const staredMail = (mail: any) =>
  api.delete(url.STARED_MAIL, { headers: { mail } });
export const trashMail = (mail: any) =>
  api.delete(url.TRASH_MAIL, { headers: { mail } });

// delete Mail
export const deleteMail = (mail: any) =>
  api.delete(url.DELETE_MAIL, { headers: { mail } });
...
export const getWalletActivities = () =>
  api.get(url.GET_WALLET_ACTIVITIES, null);
...
export const deleteTasks = (data: any) =>
  api.delete(url.DELETE_TASKS, { headers: { data } });
...
//Delete Contact User
export const deleteUsers = (users: any) =>
  api.delete(url.DELETE_USERS, { headers: { users } });

export const getUserProfile = () => api.get(url.GET_USER_PROFILE, null);

export const getEarningChartsData = (month: any) =>
  api.get(`${url.GET_EARNING_DATA}/${month}`, { params: { month } });

export const getTopSellingData = (month: any) =>
  api.get(`${url.TOP_SELLING_DATA}/${month}`, { params: { month } });
...
// Order List
export const getCryptoOrderList = () =>
  api.get(url.GET_CRYPTO_ORDRER_LIST, null);

// Invoice
//api.get Invoice
export const getInvoices = () => api.get(url.GET_INVOICES, null);

export const getInvoicesDetail = (id: number) =>
  api.get(`${url.GET_INVOICE_DETAIL}/${id}`, { params: { id } });
...
// dashbaord
//Email Chart
export const getDashboardEmailChart = (chartType: any) =>
  api.get(`${url.GET_DASHBOARD_EMAILCHART}/${chartType}`, { param: chartType });

//latest transaction
export const getTransaction = () => api.get(url.GET_TRANSACTION, null);

// Wallet Balance
export const getWalletBalance = (roomId: any) =>
  api.get(`${url.GET_WALLENT_BALANCE}/${roomId}`, { params: { roomId } });

// Visitors
export const getVisitors = (roomId: any) =>
  api.get(`${url.GET_BLOG_VISITORS}/${roomId}`, { params: { roomId } });

// statistics Applications
export const getStatisticsApplications = (roomId: any) =>
  api.get(`${url.GET_STATISTICS_APPLICATION}/${roomId}`, {
    params: { roomId },
  });
